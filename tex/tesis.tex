...
Los parámetros que definen la arquitectura del modelo se denominan hiperparámetros y, por lo tanto, este proceso de búsqueda de la arquitectura del modelo ideal se denomina ajuste de hiperparámetros o \textit{hyperparameters tuning}. \citep{medium-hpt}
...
\begin{enumerate}
	...
	\item Definir un método para muestrear valores de hiperparámetros.
	\item Definir un criterio evaluativo para juzgar el modelo.
\end{enumerate}

En nuestro caso ya cumplimos los pasos 1 y 4, necesitaríamos definir lo que necesitamos en los pasos restantes, pero para ello necesitaremos una librería de ajuste de hiperparámetros.

Existen múltiples librerías disponibles para esta tarea tanto de uso comercial como gratuitas, tales como \textit{Ray-Tune, Optuna, Hyperopt, mlmachine, Polyaxon, BayesianOptimization, Talos, SHERPA, Scikit-Optimize, GPyOpt}, etc. Sin embargo por facilidad de uso optamos por \textit{Keras-tuner}, que además cuenta con el respaldo de \textit{TensorFlow}.
...
Su propósito es realizar la búsqueda de hiperparámetros en el espacio de búsqueda definido, y como se aclaró en la sección anterior es posible crearlos de forma personalizada pero los métodos ofrecidos son más que suficientes para nuestra tarea.
